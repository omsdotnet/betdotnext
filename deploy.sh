...
./rdeploy/rdeploy --username root --host $server --private-key keys/key --passphrase $code --command "docker-compose -f /home/dotnext/docker-compose.yml up -d && docker system prune"